...
async function upgradeAll() {
  ...
  for (const [idx, song] of songs.entries()) {
    ...
    const title = ytMeta && ytMeta.title ? ytMeta.title : song.title || '(sem título)';
    console.log(`[Processando] ${idx + 1}/${songs.length}: ${title} (${videoId})`);
    const channel = ytMeta && (ytMeta.channel || ytMeta.uploader) ? (ytMeta.channel || ytMeta.uploader) : song.artist || '';
